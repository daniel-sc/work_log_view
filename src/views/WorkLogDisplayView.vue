  ...
  <div class="app-container">
    ...
    <div v-if="weeks.length > 0" class="week-select">
      ...
      <select id="weekSelect" v-model="selectedWeekStart">
        <option v-for="week in weeks" :key="week.weekStart.toISOString()" :value="week.weekStart.toISOString()">
...
import {
  ...
  groupBlocksByWeek,
  type CsvEntry,
  type WeekData, type AggregatedBlock
} from '../aggregator'
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (savedData) {
        try {
          const data = JSON.parse(savedData, (key, value) => (typeof value === 'string' && value.match(/^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d[.:+\dZ]+$/)) ? new Date(value) : value);
          if (data.blocks && data.weeks) {
            blocks.value = data.blocks;
            weeks.value = data.weeks;
            // Automatically select the latest week.
            if (data.weeks.length > 0) {
              const latestWeek = weeks.value.reduce((prev, curr) => (curr.weekStart > prev.weekStart ? curr : prev));
    ...
    watch(idleThreshold, (newVal) => {
      ...
      localStorage.setItem('worklogConfig', JSON.stringify(config, (key, value) => (value instanceof Date) ? value.toISOString() : value));
    ...
    const updateAggregation = () => {
      ...
      if (newWeeks.length > 0) {
        const latestWeek = newWeeks.reduce((prev, curr) => (curr.weekStart > prev.weekStart ? curr : prev));
        selectedWeekStart.value = latestWeek.weekStart.toISOString();
      }
    ...
    watch([weeks, selectedWeekStart], () => {
      selectedWeek.value = weeks.value.find(week => week.weekStart.toISOString() === selectedWeekStart.value) || null;
    }, { immediate: true });
